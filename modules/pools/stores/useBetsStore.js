...
// Configuração personalizada para usar IndexedDB em vez de LocalStorage
// Isso torna os dados persistentes e menos acessíveis via console/scripts simples
const idbStorage = {
  getItem: async (name) => {
    return new Promise((resolve) => {
      const request = indexedDB.open("SubpainelDezenasDB", 1);
  ...
  persist(
    (set, get) => ({
      ...
      setBall: (ball) =>
        set((state) => {
          if (state.selectedBalls.includes(ball)) {
            toast.error(`A bola ${ball} já foi selecionada!`, {
              duration: 4000,
            });
            return state;
          }
      ...
      addBet: (gameNumbers) => {
        ...
        set((state) => {
          ...
          const existingTicketIndex = state.tickets.findIndex(
            (t) =>
              t.contest_id === activeContest.id && t.pool_id === activePool.id,
          );
      ...
      removeBall: (ball) => {
        ...
        toast.error(`A bola ${ball} foi removida!`, {
          duration: 4000,
          icon: "🟠",
        });
      },
    ...
    {
      name: "bets-storage", // Nome da chave no IndexedDB
